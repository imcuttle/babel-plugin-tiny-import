    ...
    ImportSpecifier(path, map) {
        if (t.isIdentifier(path.node.imported)) {
            map[path.node.local.name] = {
                value: t.isIdentifier(path.node.imported)
    ...
    return {
        ...
        visitor: {
            Program: {
                enter(path, state) {
                    var opts = state.opts;
                    if (typeof opts.moduleMapper === 'string'
                        && opts.easyModuleMapper && opts.easyModuleMapper.enable) {
                        var stat = fs.statSync(opts.moduleMapper);
                        var basename =
                                opts.easyModuleMapper.basename == null
                                || typeof opts.easyModuleMapper.basename !== 'string'
            ...
            ImportDeclaration(path, state) {
                if (
                    !match(state.opts.test, path.node.source.value)
                ...
                if (!isEmpty(split(map, this.ignoreMap))) {
                    path.replaceWithMultiple(
                        getImportDeclarationMultipleNodes(
                            map, state.opts.moduleMapper, path.node.source.value,
                            this
                        )
                    );
                }
                ...
                path.traverse({
                    VariableDeclarator(path, state) {
                        ...
                        if (
                            ...
                            path.replaceWithMultiple(list.map(function (obj) {
                                var ret = moduleMapper(obj.imported, moduleName);

                                var callExpression = t.callExpression(
                                    node.init.callee,
                                    [t.stringLiteral(
                                        typeof ret === 'string' ? ret : (ret ? ret.moduleName : moduleName)
